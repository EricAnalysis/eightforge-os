  ...
  return records.filter((record) =>
    ...
    .map((record) => {
      ...
      return {
        ...
        earliest_divergence: overflows.length > 0
...
export function buildSemanticResolutionDiagnostic(input: {
  ...
  const interpretationChanged = checkpoint !== 'part-a';
  const extractionChanged = checkpoint.startsWith('part-c')
    || checkpoint === 'final';
  ...
  return {
    checkpoint_version: `phase1-v1.12.1-${checkpoint}`,
    checkpoint_kind: checkpoint === 'part-a'
      ? 'semantic_resolution_diagnosis_only'
      : checkpoint.startsWith('part-b')
        ? 'interpretation_only_measurement'
        : 'reconstruction_and_interpretation_measurement',
    shadow_only: true,
