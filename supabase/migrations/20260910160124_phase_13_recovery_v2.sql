-- Phase 13: additive candidate-based Recovery V2 on the Phase 12 tables.
-- Historical V1 rows are neither rewritten nor reinterpreted.

ALTER TABLE public.forgewing_recovery_proposals
  ADD COLUMN proposal_version integer NOT NULL DEFAULT 1 CHECK (proposal_version IN (1, 2)),
  ADD COLUMN recovery_type text NOT NULL DEFAULT 'pricing_rate_single_observation',
  ADD COLUMN selected_candidate_id text,
  ADD COLUMN recovery_candidates jsonb;
...
ALTER TABLE public.forgewing_recovery_proposals
  ...
  ALTER COLUMN selected_observation_id DROP NOT NULL,
  ADD CONSTRAINT forgewing_recovery_proposals_proposal_id_check CHECK (
    proposal_id ~ '^forgewing-proposal-(pricing-rate-cluster-[0-9a-f]{32}|recovery-v2-[0-9a-f]{64})$'),
  ADD CONSTRAINT forgewing_recovery_proposals_task_type_check CHECK (
    task_type IN ('pricing_rate_cluster_recovery', 'extraction_recovery_v2')),
...
ALTER TABLE public.forgewing_recovery_proposal_reviews
  ADD COLUMN confirmed_candidate_id text
    CHECK (confirmed_candidate_id IS NULL
      OR confirmed_candidate_id ~ '^recovery-candidate-v2-[0-9a-f]{64}$'),
  ADD CONSTRAINT forgewing_recovery_reviews_selection_shape_check CHECK (
    NOT (confirmed_observation_id IS NOT NULL AND confirmed_candidate_id IS NOT NULL)
  );

-- The Phase 12 coherence constraint reads "approving IFF an observation was
-- confirmed". A V2 review confirms a candidate rather than an observation, so
-- left as-is that constraint rejects every approving V2 review outright.
--
-- Widened, not weakened: an approving disposition must still carry exactly one
-- selection (the shape check above forbids both), and a selection must still
-- carry the server-derived authored text. Every V1 row satisfies this exactly
-- as it satisfied the original, and a V1 write is constrained identically.
ALTER TABLE public.forgewing_recovery_proposal_reviews
  DROP CONSTRAINT forgewing_recovery_reviews_confirmation_coherent,
  ADD CONSTRAINT forgewing_recovery_reviews_confirmation_coherent CHECK (
    (disposition IN ('accepted','modified'))
      = (confirmed_observation_id IS NOT NULL OR confirmed_candidate_id IS NOT NULL)
    AND (confirmed_observation_id IS NOT NULL OR confirmed_candidate_id IS NOT NULL)
      = (confirmed_raw_text IS NOT NULL)
  );

CREATE FUNCTION public.record_forgewing_recovery_proposal_v2(
...
CREATE FUNCTION public.record_forgewing_recovery_proposal_review_v2(
  p_organization_id uuid, p_proposal_id text, p_proposal_digest_sha256 text,
  p_reviewer_actor_id uuid, p_disposition text, p_confirmed_candidate_id text,
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE v_proposal record; v_existing record; v_version integer; v_confirmed_text text;
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'service_role required' USING ERRCODE = '42501';
  END IF;
  IF p_proposal_digest_sha256 !~ '^[0-9a-f]{64}$'
     OR p_review_request_digest_sha256 !~ '^[0-9a-f]{64}$'
     OR p_disposition NOT IN ('accepted','modified','rejected','deferred')
     OR length(btrim(p_reviewer_rationale)) NOT BETWEEN 1 AND 4000
     OR ((p_disposition IN ('accepted','modified')) <> (p_confirmed_candidate_id IS NOT NULL)) THEN
    RAISE EXCEPTION 'invalid recovery v2 review' USING ERRCODE = '22023';
  END IF;
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_proposal_digest_sha256, 0));
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'exact recovery proposal not found' USING ERRCODE = 'P0002'; END IF;
  ...
  END IF;
  IF p_confirmed_candidate_id IS NOT NULL THEN
    SELECT candidate->>'composedRawText' INTO v_confirmed_text
    FROM jsonb_array_elements(v_proposal.recovery_candidates) candidate
    WHERE candidate->>'candidateId' = p_confirmed_candidate_id;
    IF v_confirmed_text IS NULL THEN
      RAISE EXCEPTION 'confirmed candidate is not eligible proposal evidence' USING ERRCODE = '22023';
    END IF;
    IF (p_disposition = 'accepted') IS DISTINCT FROM
       (p_confirmed_candidate_id = v_proposal.selected_candidate_id) THEN
      RAISE EXCEPTION 'review disposition does not match candidate selection' USING ERRCODE = '22023';
    END IF;
  END IF;
  ...
  IF FOUND THEN
    IF v_existing.organization_id IS DISTINCT FROM p_organization_id
       ...
       OR v_existing.confirmed_candidate_id IS DISTINCT FROM p_confirmed_candidate_id
    ...
    RETURN QUERY SELECT v_existing.id, v_existing.review_version, v_existing.proposal_row_id,
      v_existing.confirmed_candidate_id, v_existing.confirmed_raw_text, false;
    RETURN;
  END IF;
  ...
  VALUES (p_organization_id, v_proposal.id, p_proposal_digest_sha256, v_version,
    p_reviewer_actor_id, p_disposition, NULL, p_confirmed_candidate_id,
    v_confirmed_text, btrim(p_reviewer_rationale), p_review_request_digest_sha256)
  RETURNING id INTO review_id;
  review_version := v_version; proposal_row_id := v_proposal.id;
  confirmed_candidate_id := p_confirmed_candidate_id; confirmed_raw_text := v_confirmed_text;
  inserted := true; RETURN NEXT;
END $$;

REVOKE ALL ON FUNCTION public.record_forgewing_recovery_proposal_review_v2(
  uuid,text,text,uuid,text,text,text,text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.record_forgewing_recovery_proposal_review_v2(
